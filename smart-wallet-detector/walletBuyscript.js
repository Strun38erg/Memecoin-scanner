...
async function processAndDisplayData() {
    ...
    console.log("Finished processing. Saving data to walletBuydata.json");

    try {
        const filePath = './walletBuydata.json';
        fs.writeFileSync(filePath, JSON.stringify(walletData, null, 2));
        console.log(`Wallet data saved to ${filePath}`);
    } catch (error) {
        console.error('Error saving data to walletBuydata.json:', error);
    }
